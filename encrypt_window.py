...
def image():
    def img_enc():
        encrypt_image.encrypter(passphrase, box1)

    box1 = Tk()
    box1.title('ENCRYPT IT')
    box1.geometry("600x400")
    label1 = Label(box1, text="Enter 10 digit numeric pass-phrase")
    label1.config(font=("Elephant", 10))
    label1.pack()

    enb = Button(box1, text="Choose File To Encrypt", command=img_enc)
...
def document():
    def doc_enc():
        document_encrypt.encrypter(passphrase, box1)
    box1 = Tk()
    box1.title('ENCRYPT IT')
    box1.geometry("600x400")
    label1 = Label(box1, text="Enter 10 digit numeric pass-phrase")
    ...
    enb = Button(box1, text="Choose File To Encrypt", command=doc_enc)
    enb.place(x=200, y=300)
...
def text():
    def text_enc():
        text_encrypt.encrypter(passphrase, box1)
    box1 = Tk()
    box1.title('ENCRYPT IT')
    box1.geometry("600x400")
    label1 = Label(box1, text="Enter 10 digit numeric pass-phrase")
    ...
    enb = Button(box1, text="Choose File To Encrypt", command=text_enc)
    enb.place(x=200, y=300)
...
def encrypt_menu():
    ...
    label1 = Label(box, text="What Do YOU Want To Encrypt")
